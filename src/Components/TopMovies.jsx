...
const TopMovies = () => {
  ...
  const { movies, page, searchedMovies, searchButton } = useSelector(
    (state) => state.movies
  );

  useEffect(() => {
    dispatch(getTopMovies(page));
    window.scrollTo(0,0);
    ...
    <div className="bg-[#272b34] text-white py-20 px-2">
      ...
      {!searchButton ? (
        <div className="space-x-2 mt-20 w-full text-center">
          ...
          {page - 5 > 0 && (
            <button onClick={() => dispatch(setPage(page - 5))}>
              {page - 5}
            </button>
          )}
          {page - 4 > 0 && (
            <button onClick={() => dispatch(setPage(page - 4))}>
              {page - 4}
            </button>
          )}
          {page - 3 > 0 && (
            <button onClick={() => dispatch(setPage(page - 3))}>
              {page - 3}
            </button>
          )}
          {page - 2 > 0 && (
            <button onClick={() => dispatch(setPage(page - 2))}>
              {page - 2}
            </button>
          )}
          {page - 1 > 0 && (
          ...
          <button
            className="bg-green-500 rounded-[100%] px-2"
          ...
          {page < 5 && (
            <>
              <button onClick={() => dispatch(setPage(page + 6))}>
                {page + 6}
              </button>
              <button onClick={() => dispatch(setPage(page + 7))}>
                {page + 7}
              </button>
              <button onClick={() => dispatch(setPage(page + 8))}>
                {page + 8}
              </button>
              <button onClick={() => dispatch(setPage(page + 9))}>
                {page + 9}
              </button>
            </>
          )}
